...
İzinPro; personel izin taleplerini oluşturmak, takip etmek ve yönetmek için hazırlanmış yerel bir web uygulamasıdır. Sistem internet üzerinde yayınlanmaz; Node.js ile bilgisayarda çalışır ve bütün bilgiler yerel SQLite veritabanında saklanır.
...
backend/                Node.js API ve SQLite veritabanı
frontend/               Programın ekranları ve tasarımı
tests/                  Otomatik sistem testleri
package.json            Node.js komutları ve bağımlılıkları
```

Teknik akış `Route → Controller → Service → Repository → SQLite` şeklindedir. Backend Node.js/Express ile, kullanıcı arayüzü HTML, CSS ve JavaScript ile hazırlanmıştır.
