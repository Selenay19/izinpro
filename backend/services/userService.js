...
async function create(input) {
  ...
  const email = generateAddress(firstName, lastName, (candidate) => Boolean(userRepository.findByEmail(candidate)));
  const role = ensureRole(input.role || 'PERSONNEL');
  const position = positionService.getActive(input.positionId);
...
function setStatus(requester, idInput, active) {
  ...
  const user = userRepository.findById(id);
  if (!user) throw new AppError('Kullanıcı bulunamadı.', 404);
  if (!isActive && user.role === 'ADMIN' && userRepository.countActiveAdmins() <= 1) {
...
async function resetPassword(idInput, newPassword) {
  ...
  userRepository.setPassword(id, await bcrypt.hash(validated, 10), true, credentialVault.encrypt(validated));
}

function getTemporaryCredentials(idInput) {
  const id = validate.positiveInteger(idInput, 'Kullanıcı');
  const user = userRepository.findById(id, true);
  if (!user) throw new AppError('Kullanıcı bulunamadı.', 404);
...
function remove(requester, idInput) {
  const id = validate.positiveInteger(idInput, 'Kullanıcı');
  if (requester.id === id) throw new AppError('Kendi hesabınızı silemezsiniz.', 409);
  const user = userRepository.findById(id);
  if (!user) throw new AppError('Kullanıcı bulunamadı.', 404);
  if (user.role === 'ADMIN' && user.isActive && userRepository.countActiveAdmins() <= 1) {
    throw new AppError('Sistemdeki son aktif admin silinemez.', 409);
  }
  const references = userRepository.referenceCounts(id);
