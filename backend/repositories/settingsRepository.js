...
function get() {
  return map(getDatabase().prepare('SELECT * FROM app_settings WHERE id = 1').get());
}
...
function clearOperations(currentSessionId) {
  const db = getDatabase();
  db.transaction(() => {
...
function factoryReset(adminId, currentSessionId) {
  const db = getDatabase();
  db.transaction(() => {
