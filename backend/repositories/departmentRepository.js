...
function remove(id) {
  getDatabase().prepare('DELETE FROM departments WHERE id = ?').run(id);
}

function userCount(id) {
  return getDatabase().prepare('SELECT COUNT(*) AS count FROM users WHERE department_id = ?').get(id).count;
}
