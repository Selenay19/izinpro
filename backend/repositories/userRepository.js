...
function setPassword(id, passwordHash, mustChangePassword = false, temporaryPasswordSecret = null) {
  getDatabase().prepare(
...
function referenceCounts(id) {
  const db = getDatabase();
  return {
    ownRequests: db.prepare('SELECT COUNT(*) AS count FROM leave_requests WHERE user_id = ?').get(id).count,
    approvals: db.prepare('SELECT COUNT(*) AS count FROM approval_history WHERE manager_id = ?').get(id).count,
    approvedRequests: db.prepare('SELECT COUNT(*) AS count FROM leave_requests WHERE approved_by = ?').get(id).count,
    managedDepartments: db.prepare('SELECT COUNT(*) AS count FROM departments WHERE manager_id = ?').get(id).count
  };
}
